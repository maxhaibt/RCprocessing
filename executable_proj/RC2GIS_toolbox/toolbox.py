...
def log_transform_diff(src_A, dst_A):
    diffs = []
    if not np.isclose(src_A.a, dst_A.a): diffs.append(f"a (px width): {src_A.a} -> {dst_A.a}")
    if not np.isclose(src_A.e, dst_A.e): diffs.append(f"e (px height): {src_A.e} -> {dst_A.e}")
    if not np.isclose(src_A.b, dst_A.b): diffs.append(f"b (x-skew): {src_A.b} -> {dst_A.b}")
    if not np.isclose(src_A.d, dst_A.d): diffs.append(f"d (y-skew): {src_A.d} -> {dst_A.d}")
...
def export_raster_gpkg(src_tif_path, gpkg_path, *, nodata=None,
                       ...
                       quality=90, zlevel=6):
    """
    Convert a GeoTIFF to a raster GeoPackage with tiles + pyramids.
    DEM/spectral -> PNG tiles (lossless); truecolor -> JPEG tiles (compact).
    """
    co = [
...
def process_files_truecolor(file_paths, progress, status, should_continue, root):
    """
    Truecolor RGB orthos:
    - warp to north-up
    - bilinear resampling (use Resampling.cubic if you prefer smoother)
